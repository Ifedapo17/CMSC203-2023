...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int min_Value = intArray[0][column];
		for (int row = 1; row < intArray.length; row++){
			if (intArray[row][column] < min_Value) {
				min_Value = intArray[row][column];
			}
			animate(row, column, min_Value);
		}
		return min_Value; // replace this line with your return statement
   }
		...
		for (int row = 0; row < intArray.length; row++) {
			for (int col = 0; col < intArray[row].length; col++) {
				if (intArray[row][col] == value)
					num_Repeat++;
				animate(row, col, num_Repeat);
			}
		}
		return num_Repeat;// replace this line with your return statement
   }
